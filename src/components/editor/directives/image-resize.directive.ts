...
const isEditableContent = (node): boolean => {
  let editableClassName = 'ax__editor__content';
  try {
...
const documentClickHandle = ({ target }) => {
  if (!isEditableContent(target)) {
    document.querySelectorAll('.ax__image__editable').forEach($resize => $resize.remove());
  }
}
...
const __getParentTd = (node): HTMLElement => {
  while (node.tagName !== 'TD' && node.tagName !== 'TH' && !node.className.includes('ax__editor__content')) {
    node = node.parentElement;
  }
  return node;
}
const __getTable = (node): HTMLTableElement => {
  while (node.tagName !== 'TABLE' && !node.className.includes('ax__editor__content')) {
    node = node.parentElement;
  }
  return node;
}